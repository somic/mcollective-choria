    ...
    class Playbook
      class DataStores
        class ShellDataStore < Base
          def write(key, value)
            run("write", key, "CHORIA_DATA_VALUE" => value).stdout.chomp
          end

          def delete(key)
            run("delete", key).stdout.chomp
          end
          ...
          def run(action, key, environment={})
            ...
            command = "%s --%s %s" % [@command, action, arguments.join(" ")]
            ...
            unless shell.status.exitstatus == 0
              ...
              raise("Could not %s key %s, got exitcode %d" % [action, key, shell.status.exitcode])
            end
          ...
          def validate_key(key)
            raise("Valid keys have to match ^[a-zA-Z0-9_-]+$") unless key.match(/^[a-zA-Z0-9_-]+$/)
          end
